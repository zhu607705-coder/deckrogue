...
async function main(): Promise<void> {
  ...
  const stages: Array<{ name: string; command: string; env?: Record<string, string> }> = [
    ...
    { name: 'Runtime V2 Adapter Differential Parity', command: 'npm run check:runtime-v2-adapter-differential-parity 2>&1' },
    ...
    { name: 'Check Route State Save Load Parity', command: 'npm run check:route-state-save-load-parity 2>&1' },
    { name: 'Check Event Choice Reinforcement', command: 'npm run check:event-choice-reinforcement 2>&1' },
    { name: 'Check Rest Route Reinforcement', command: 'npm run check:rest-route-reinforcement 2>&1' },
    { name: 'Check Shop Route Reinforcement', command: 'npm run check:shop-route-reinforcement 2>&1' },
