...
async function main(): Promise<void> {
  ...
  const stages: Array<{ name: string; command: string; env?: Record<string, string> }> = [
    ...
    { name: 'Runtime V2 Unit Tests', command: 'npm run test:runtime-v2:ts 2>&1' },
    { name: 'Supplemental Unit Tests', command: 'npm run test:supplemental-units 2>&1' },
    { name: 'Python Runtime Tests', command: 'npm run test:runtime-v2:py 2>&1' },
    { name: 'Accept Runtime V2 Parity', command: 'npm run accept:runtime-v2-parity 2>&1' },
