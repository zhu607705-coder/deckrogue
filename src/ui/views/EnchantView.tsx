...
import { getCardEnchantmentDefById } from '@/content/narrative/numericSystem';
...
export function EnchantView({ engine }: { engine: GameEngine }) {
  ...
  const enchantableCards = engine.state.player.deck.filter((card) => {
    const runCard = (card as any);
    const hasNoEnchantment = (card.type === 'Attack' || card.type === 'Skill') && (!Array.isArray(runCard.persistentEnchantments) || runCard.persistentEnchantments.length === 0);
